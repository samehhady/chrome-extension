...
export default class RouletteWheel {
    ...
    { // Clean items:
      ...
      if (this.itemColorSet.length) {
        ...
        for (let i = 0; i < items.length; i++) {
          ...
          if (!items[i].color) {
            items[i].color = c;
            items[i].borderColor = 'red'
            items[i].borderWidth = 10
          }
        }
      } else {
  ...
  drawFrame(now) {
    ...
    ctx.beginPath();
    ctx.moveTo(this.canvasCenterX - 20, 10);
    ctx.lineTo(this.canvasCenterX + 20, 10);
    ctx.lineTo(this.canvasCenterX, 30);
    ctx.closePath();
